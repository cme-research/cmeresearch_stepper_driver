...
typedef struct {
	...
	uint8_t video_freeze_during_ffc;
	uint8_t ffc_desired;
	uint32_t elapsed_time_since_last_ffc;
	uint32_t desired_ffc_period;
	uint8_t explicit_cmd_to_open;
	uint16_t desired_ffc_temp_delta;
	uint16_t imminent_delay;
} ATTRIBUTE_PACKED SetFFCShutterMode_Request;

typedef struct {
	PacketHeader header;
} ATTRIBUTE_PACKED GetFFCShutterMode_Request;
...
typedef struct {
	...
	uint16_t desired_ffc_temp_delta;
	uint16_t imminent_delay;
} ATTRIBUTE_PACKED GetFFCShutterMode_Response;

typedef struct {
	PacketHeader header;
} ATTRIBUTE_PACKED RunFFCNormalization_Request;
...
void thermal_imaging_create(ThermalImaging *thermal_imaging, const char *uid, IPConnection *ipcon) {
	...
	device_create(thermal_imaging, uid, ipcon_p, 2, 0, 2, THERMAL_IMAGING_DEVICE_IDENTIFIER);
	...
	device_p->response_expected[THERMAL_IMAGING_FUNCTION_SET_FFC_SHUTTER_MODE] = DEVICE_RESPONSE_EXPECTED_FALSE;
	device_p->response_expected[THERMAL_IMAGING_FUNCTION_GET_FFC_SHUTTER_MODE] = DEVICE_RESPONSE_EXPECTED_ALWAYS_TRUE;
	device_p->response_expected[THERMAL_IMAGING_FUNCTION_RUN_FFC_NORMALIZATION] = DEVICE_RESPONSE_EXPECTED_FALSE;
...
int thermal_imaging_set_ffc_shutter_mode(ThermalImaging *thermal_imaging, uint8_t shutter_mode, uint8_t temp_lockout_state, bool video_freeze_during_ffc, bool ffc_desired, uint32_t elapsed_time_since_last_ffc, uint32_t desired_ffc_period, bool explicit_cmd_to_open, uint16_t desired_ffc_temp_delta, uint16_t imminent_delay) {
	DevicePrivate *device_p = thermal_imaging->p;
	SetFFCShutterMode_Request request;
	int ret;
	...
	ret = packet_header_create(&request.header, sizeof(request), THERMAL_IMAGING_FUNCTION_SET_FFC_SHUTTER_MODE, device_p->ipcon_p, device_p);

	if (ret < 0) {
		return ret;
	}

	request.shutter_mode = shutter_mode;
	request.temp_lockout_state = temp_lockout_state;
	request.video_freeze_during_ffc = video_freeze_during_ffc ? 1 : 0;
	request.ffc_desired = ffc_desired ? 1 : 0;
	request.elapsed_time_since_last_ffc = leconvert_uint32_to(elapsed_time_since_last_ffc);
	request.desired_ffc_period = leconvert_uint32_to(desired_ffc_period);
	request.explicit_cmd_to_open = explicit_cmd_to_open ? 1 : 0;
	request.desired_ffc_temp_delta = leconvert_uint16_to(desired_ffc_temp_delta);
	request.imminent_delay = leconvert_uint16_to(imminent_delay);
...
int thermal_imaging_get_ffc_shutter_mode(ThermalImaging *thermal_imaging, uint8_t *ret_shutter_mode, uint8_t *ret_temp_lockout_state, bool *ret_video_freeze_during_ffc, bool *ret_ffc_desired, uint32_t *ret_elapsed_time_since_last_ffc, uint32_t *ret_desired_ffc_period, bool *ret_explicit_cmd_to_open, uint16_t *ret_desired_ffc_temp_delta, uint16_t *ret_imminent_delay) {
	...
	ret = packet_header_create(&request.header, sizeof(request), THERMAL_IMAGING_FUNCTION_GET_FFC_SHUTTER_MODE, device_p->ipcon_p, device_p);
	...
	*ret_shutter_mode = response.shutter_mode;
	*ret_temp_lockout_state = response.temp_lockout_state;
	*ret_video_freeze_during_ffc = response.video_freeze_during_ffc != 0;
	*ret_ffc_desired = response.ffc_desired != 0;
	*ret_elapsed_time_since_last_ffc = leconvert_uint32_from(response.elapsed_time_since_last_ffc);
	*ret_desired_ffc_period = leconvert_uint32_from(response.desired_ffc_period);
	*ret_explicit_cmd_to_open = response.explicit_cmd_to_open != 0;
	*ret_desired_ffc_temp_delta = leconvert_uint16_from(response.desired_ffc_temp_delta);
	*ret_imminent_delay = leconvert_uint16_from(response.imminent_delay);

	return ret;
}

int thermal_imaging_run_ffc_normalization(ThermalImaging *thermal_imaging) {
	DevicePrivate *device_p = thermal_imaging->p;
	RunFFCNormalization_Request request;
	int ret;
	...
	ret = packet_header_create(&request.header, sizeof(request), THERMAL_IMAGING_FUNCTION_RUN_FFC_NORMALIZATION, device_p->ipcon_p, device_p);
