    ...
    if(device_identifier == SILENT_STEPPER_V2_DEVICE_IDENTIFIER) {
		...
		std::string hw_bricklet_position_str(1, position);
 		if (hw_bricklet_position_str == stepperDriver->brick_position_) {
			...
			silent_stepper_v2_set_enabled(&(stepperDriver->brickletStepperV2_), true);
...
void StepperDriver::stepperDisabled() {
	fprintf(stderr, "stepperDisabled\n");
	silent_stepper_v2_set_enabled(&brickletStepperV2_, false);
    stepper_enabled_ = false;
}
...
bool StepperDriver::consume_voltage_recovered() {
  if (reset_after_under_voltage_triggered_) {
    reset_after_under_voltage_triggered_ = false;
    return true;
  }
  return false;
